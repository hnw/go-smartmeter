...
func TestParseFrame(t *testing.T) {
	...
	expectedEPC := LvSmartElectricEnergyMeterInstantaneousCurrent
...
func TestNewFrame(t *testing.T) {
	frame := NewFrame(LvSmartElectricEnergyMeter, Get, []*Property{
		NewProperty(LvSmartElectricEnergyMeterInstantaneousElectricPower, nil),
		NewProperty(LvSmartElectricEnergyMeterInstantaneousCurrent, nil),
	})
	frame.TID = 0
	s := frame.Build()
...
func TestEchoFrameCorrespondTo(t *testing.T) {
	...
	if !req.CorrespondTo(res) {
		t.Errorf(
			"echoFrame.CorrespondTo() error: '%s' vs '%s'",
			hex.EncodeToString(req.Build()),
			hex.EncodeToString(res.Build()),
		)
	}
